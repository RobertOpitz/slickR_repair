...
slick_list <- function(...)
{
  ...
  htmltools::tagList(lapply(dots, htmltools::tags$div))
}
...
slick_div_chr <- function(x, css, type = 'img', links = NULL, is_file = TRUE)
{
  if (is_file &&
      utils::file_test('-f', x)) {
    x_img   <- readImage(x)
    ...
    ret_tag <- switch(type,
                      'p'        = slick_tag(type, 
                                             varArgs = list(x, 
                                                            style = css)),
                      'iframe'   = slick_tag(type, 
                                             varArgs = list(srcdoc = x, 
                                                            style  = css)),
                      'img-lazy' = slick_tag('img', 
                                             varArgs = list('data-lazy' = x, 
                                                            style       = css)),
                      slick_tag(type, varArgs = list(src   = x, 
                                                     style = css)))
